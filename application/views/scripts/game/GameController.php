<?php
	class GameController extends Zend_Controller_Action {
		...
		function popupAction(){		
			...
			$channelO=$_GET['channel']; 	
			$regionO=$_GET['region'];
			$product=$_GET['product'];

			$outcomes=new Model_DbTable_Outcomes_St_Units();
			$this->view->game=$games->getGame($_GET['game_id']);
			$this->view->stocks_units=$stocks_units=$outcomes->getStockByMarket($_GET['game_id'], $_GET['round']-1, $_GET['company_id'], $_GET['product'], $_GET['region'], $_GET['channel']);
			$this->view->regions=$games->getRegions($_GET['game_id']);
			$this->view->channels=$games->getChannels($_GET['game_id']);;
			$this->view->n_products=$games->getNumberOfProductsAvailable($_GET['game_id'], $_GET['round'], $_GET['company_id']);

			$distribution= explode (";" ,  $_GET['distributionInformation']);
			$contador=0;
			...
			for ($channel=1; $channel<=$n_channels; $channel++){
				for ($region=1; $region<=$n_regions; $region++){
					if((count($distribution)-1)==0){
							$distributionInformation['channel_'.$channel]['region_'.$region]= "0";
					}
					for ($i=0; $i<count($distribution)-1; $i++ ){
						if($_GET['product']==substr($distribution[$i], 0, 1) && $_GET['channel'] ==substr($distribution[$i], 2, 1)&&$_GET['region'] ==substr($distribution[$i], 1, 1)&&$channel ==substr($distribution[$i], 4, 1)&&$region ==substr($distribution[$i], 3, 1)){
							$distributionInformation['channel_'.$channel]
							['region_'.$region]= substr($distribution[$i], 5);
							$contador+=substr($distribution[$i], 5);
						}elseif($distributionInformation['channel_'.$channel] ['region_'.$region] == null or $distributionInformation['channel_'.$channel]['region_'.$region] == ""){
										$distributionInformation['channel_'.$channel]['region_'.$region] = "0";
						}
					}
				}
			}
			if($contador!=$stocks_units){
				$distributionInformation['channel_'.$_GET['channel']]['region_'.$_GET['region']]= $stocks_units-$contador;
			}
			$this->view->unitsStockDistributionDecision=$distributionInformation;
			
			if ($this->getRequest()->isPost()){	
				$boolean= true;		
				$postData=$this->getRequest()->getPost();

				if (isset ($postData['unitsStockDistribution'])){
					$st_decisionDistributionData=$postData['unitsStockDistribution'];
					$contador= 0;

					for ($channel=1; $channel<=$n_channels; $channel++){
						for ($region=1; $region<=$n_regions; $region++){
							$contador+= intval($st_decisionDistributionData['channel_'.$channel] ['region_'.$region]);

							if(intval($st_decisionDistributionData['channel_'.$channel] ['region_'.$region])  < 0){
								print '<script language="JavaScript">'; 
								print 'alert("No se aceptan número negativos. Por favor, distribuya correctamente el stock");'; 
								print '</script>'; 
								$boolean= false;
					...
					if ($contador!=$stocks_units && $boolean){
						print '<script language="JavaScript">'; 
						print 'alert("La distribución no es correcta. Por favor, distribuya correctamente el stock");'; 
						print '</script>';
						$boolean= false;
					}
				}
				/*$situation_min =$n_regions*$n_channels*(intval($_GET['product'])-1)+intval($_GET['product'])-1;
				print '<script language="JavaScript">'; 
						print '$iLeft = $situation_min+$n_regions*($channelO-1)+$regionO-1;';
						print 'window.opener.document.getElementById('unitsStockLeftField').getElementsByTagName("input")[$iLeft].value=0;'; 
						print '</script>';

				$editdecision->view->unitsStockDistributiontDecision="Hola, funciono";*/

				print '<script language="JavaScript">'; 
				print 'validatePopupForm("<?php echo $product;  echo $regionO; echo $channelO; echo $n_regions; echo $n_channels;?>"");';
				print '</script>';
	...
	function validatePopupForm(origin){
		...
		function editdecisionAction(){
			...
			if ($this->getRequest()->isPost()){	
				...
				$st_decisionResult=$postData['stock']['unitsStockResult'];
				$contadorResult=0;
				$contadorCache=0;
				...
				if($error == true){
					...
					if (isset ($postData['stock'])){
						...
						//Opción 2
						/*for ($i=0; $i<count($distribution)-1; $i++ ){
							$distributionInformation[$i]= array('product'=>substr($distribution[$i], 0, 1), 'channelO'=>substr($distribution[$i], 2, 1), 'regionO'=>substr($distribution[$i], 1, 1), 'channelD'=>substr($distribution[$i], 4, 1), 'regionD'=>substr($distribution[$i], 3, 1), 'value'=>substr($distribution[$i], 5));
						}*/
						//Opción 1
						$distributionInformation=array();
						for ($product=1; $product<=$n_products; $product++){
						...
						$st_decisionData['stock']['unitsStockResult']=$postData['stock']['unitsStockResult'];
					...
					if(!$error){
						...
						if (isset ($postData['production_decision'])){
							...
							var_dump($functionalityInformation);
			...
			if ($st_decisions->existsPrevious($_GET['game_id'], $_GET['company_id'], $_GET['round'])){
				...
				//Opción 2
				/*for ($i=0; $i<count($st_lastDecision['distributionStock'])-1; $i++){
					$st_distribuion_lastDecision=$st_distribuion_lastDecision.(strval($st_lastDecision['distributionStock'][$i]['product']).strval($st_lastDecision['distributionStock'][$i]['regionO']).strval($st_lastDecision['distributionStock'][$i]['channelO']).strval($st_lastDecision['distributionStock'][$i]['regionD']).strval($st_lastDecision['distributionStock'][$i]['channelD']).strval($st_lastDecision['distributionStock'][$i]['value']).";");
				}*/
						...
						while (isset($units_channelO['regionO_number_'.$regionO_number])){
							...
							while (isset($units_regionO['channelD_number_'.$channelD_number])){
								...
								while (isset($units_channelD['regionD_number_'.$regionD_number])){
									...
									if ($units<> "0"){
										//$st_distribuion_lastDecision=$st_distribuion_lastDecision.(strval($distributionData['product']).strval($distributionData['channelO']).strval($distributionData['regionO']).strval($distributionData['channelD']).strval($distributionData['regionD']).strval($distributionData['value']).";");
...
*/
			...
			if ($this->getRequest()->isPost()){	
				...
				if (! $games->existsByName($gameFormData['name'])){
					...
					$game_data=array('name'=>$gameFormData['name'], 'description'=>$gameFormData['description'], 'is_new'=>1, 
									 ...
									 'n_functionalities'=>$gameFormData['n_functionalities']);
					var_dump($gameFormData['n_functionalities']);
					//VERO
		...
		function downloadAction() {
			...
			$col=1;
			foreach ($companies as $company){
				$worksheet->setCellValueByColumnAndRow($col, $offset, $outcomes_costs[$company['id']]['fi_debt_costs_lt']+$outcomes_costs[$company['id']]['fi_debt_costs_st']);
				$col++;
			}
			$offset++;
			...
			$col=1;
			foreach ($companies as $company){
				$ebt[$col]=$ebitda[$col]-$outcomes_costs[$company['id']]['fi_debt_costs_st']-$outcomes_costs[$company['id']]['fi_debt_costs_lt']-$amortization[$col];
