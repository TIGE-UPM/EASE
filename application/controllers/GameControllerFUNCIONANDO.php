	...
	class GameController extends Zend_Controller_Action {
		...
		function editdecisionAction(){
			...
			if($round_actual>1){
				...
				$stockValue=$outcomes->getStocksByProducts($_GET['game_id'], $round_previous, $_GET['company_id'], 1);
				//var_dump($stockValue);
				//var_dump($result);
				//$this->view->stockValue=$stockValue;
			}*/
			//VERO
			...
			if ($this->getRequest()->isPost()){			
				...
				//VERO
				if (isset ($postData['unitsStock'])){
					$st_decisionData=$postData['unitsStock'];
			...
			//human resources
				...
				if($round_actual>1){
					...
					//VERO
					$st_decisions_prev=$st_decisions->getDecisionArray($_GET['game_id'], $_GET['company_id'], $round_previous);
					//VERO
					...
					//VERO
					$this->view->unitsStockDecision=$st_decisions_prev['unitsStock'];
					//VERO
			...
			if ($st_decisions->existsPrevious($_GET['game_id'], $_GET['company_id'], $_GET['round'])){
				...
				$this->view->unitsStockDecision=$st_lastDecision['unitsStock'];
			}//VERO
...
*/
		...
		function viewAction($params=null){
			...
			if (isset ($_GET['game_id'])){
				...
				//VERO
				$this->view->stock=new Model_DbTable_Decisions_Stock();
				//VERO
